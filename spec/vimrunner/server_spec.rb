  ...
  describe Server do
    ...
    describe "#start" do
      it "starts a vim server process" do
        begin
          ...
          server.kill
          expect(server.serverlist).not_to include(server.name)
        end
      end
    ...
    describe "connecting to an existing server" do
      ...
      it "returns a client" do
        expect(second_server.connect).to be_kind_of(Vimrunner::Client)
        expect(second_server.connect!).to be_kind_of(Vimrunner::Client)
      end
    ...
    describe "#new_client" do
      it "returns a client" do
        expect(server.new_client).to be_kind_of(Vimrunner::Client)
      end
    ...
    describe "#remote_expr" do
      it "uses the server's executable to send remote expressions" do
        allow(server).to receive(:execute).
